...
//                        System.out.println("I M STUCK");
                        tryMove(dir,5,30);
                        motionType = 'd';
//                        rc.setIndicatorLine(here,here.add(dir,20f),30,30,30);
                    }
                }
            }
            else
            {
                ...
                if(rc.canMove(dir) && rc.senseNearbyTrees(here.add(dir,2*myType.bodyRadius),2*myType.bodyRadius,null).length==0)
                {
                    motionType = 'd';
                    tryMove(dir);
...
//            rc.setIndicatorDot(target,255,255,255);
    ...
    public static boolean willMyBulletHit(RobotInfo robotInfo)
    {
        here = rc.getLocation();
        Direction fireDirection = here.directionTo(robotInfo.location);
        for(float i = here.distanceTo(robotInfo.location)-robotInfo.getRadius(); i-->1;)
        {
            if(rc.senseNearbyTrees(here.add(fireDirection,i),GameConstants.BULLET_SPAWN_OFFSET,null).length>0)
            {
                return false;
            }
            if(rc.senseNearbyRobots(here.add(fireDirection,i),GameConstants.BULLET_SPAWN_OFFSET,them).length>0)
            {
                return true;
            }
        }
