...
public class ArchonBot extends BaseBot
{
    ...
    public static void runArchon() throws GameActionException
    {
        while (true)
        {
            try
            {
                ...
                if(rc.readBroadcast(BROADCAST_INITIAL_ARCHONS) == 0)
                {
                    broadcastInitialArchonLocations();
                    rc.broadcast(BROADCAST_INITIAL_ARCHONS,1);
                }
                updateRobotInfos();
                ...
                if (dir == null)
                    dir = getAnySpawnDirectionPossible();
    ...
    private static void broadcastInitialArchonLocations() throws GameActionException
    {
        for(MapLocation loc: theirInitialArchonLocations)
        {
            RobotInfo r = new RobotInfo(1,them,RobotType.ARCHON,loc,myType.maxHealth,0,0);
            broadcastEnemyLocation(r);
        }
    }

    private static Direction getAnySpawnDirectionPossible()
    {
        here = rc.getLocation();
        for (int i = 0; i <= 360; i++)
        {
            Direction spawnDir = here.directionTo(closetInitalEnemyArchonLocation()).rotateLeftDegrees(i);
            if (rc.canHireGardener(spawnDir))
            {
                return spawnDir;
            }
        }
        return here.directionTo(centerOfTheirInitialArchons).rotateLeftDegrees((float) (Math.random() * (180) - 90));
    }


    private static boolean isViableToSpawnGardeners() throws GameActionException
    {
        if(rc.readBroadcast(GARDENER_ABOUT_TO_DIE_CHANNEL) == 1)
        {
            rc.broadcast(GARDENER_ABOUT_TO_DIE_CHANNEL,0);
            return true;
        }
        ...
        return (numRound - rc.readBroadcast(LAST_GARDENER_SPAWNED_CHANNEL) > 15) && (rc.getTeamBullets() > RobotType.SOLDIER.bulletCost * 1.15);
    }
}
