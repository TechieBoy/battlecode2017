...
public class GardenerBot extends BaseBot
{
    ...
    private static boolean initiallyNeedLumberJacks = rc.getRoundNum() < 80;
    ...
    public static void runGardener() throws GameActionException
    {
        try
        {
            ...
            while (true)
            {
                ...
                if (rc.getRoundNum() - ROUND_SPAWNED > (rc.readBroadcast(GARDENER_WANDER_CONSTANT_CHANNEL)))
                {
                    if (rc.senseNearbyTrees(-1, Team.NEUTRAL).length > 3)
                        initiallyNeedLumberJacks = true;
                    break;
                } else if (!rc.hasMoved())
                    wander();
            }
            ...
            try
            {
                ...
                if (initiallyNeedLumberJacks)
                {
                    if (spawnInAnyDirectionPossible(RobotType.LUMBERJACK))
                    {
                        ...
                        initiallyNeedLumberJacks = false;
                    }
    ...
    {
        return (rc.senseNearbyTrees(-1, Team.NEUTRAL).length > 4) || (rc.getRoundNum() < 400 && rc.getTreeCount() > 2);
