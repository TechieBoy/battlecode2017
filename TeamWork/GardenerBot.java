...
public class GardenerBot extends BaseBot
{
    ...
    private static boolean needExtraLumberjacks = rc.getRoundNum() < 50;
    ...
    public static void runGardener() throws GameActionException
    {
        try
        {
            ...
            {
                here = rc.getLocation();
                if ((howManyTreesCanBePlanted(here) >= 4) && here.distanceTo(closetInitalAlliedArchonLocation()) > 6)
                {
                    break;
                }
    ...
    {
        return (rc.senseNearbyTrees(-1, Team.NEUTRAL).length > 4) || (rc.getRoundNum() < 450 && rc.getTreeCount() > 2);
