...
public class GardenerBot extends BaseBot
{
    ...
    private static boolean earlyLumberjack = false;
        ...
        try
        {
            if (!tankGardener)
            {
                ...
                while (!(howManyTreesCanBePlanted(here) >= 4))
                {
                    ...
                    {
                        if (rc.senseNearbyTrees(5f, Team.NEUTRAL).length >= 4)
...
//                    }
        ...
        if (visibleEnemies.length > 0)
        {
            if (rc.getHealth() < RobotType.GARDENER.maxHealth || rc.senseNearbyBullets(4f).length > 0)
            {
                ...
                for(RobotInfo robotInfo : visibleEnemies)
                {
                    ...
                    {
                        spawned = spawnInAnyDirectionPossible(RobotType.LUMBERJACK);
                        break;
                    }
                }
                if(!spawned)
    ...
    private static boolean needLumberjacks() throws GameActionException
    {
        ...
        return (rc.senseNearbyTrees(RobotType.GARDENER.sensorRadius/2, Team.NEUTRAL).length > 3) || Math.random() < 0.35;
    }
    ...
    {
        return (rc.getRoundNum() < 100 && rc.getTreeCount() > 1);
