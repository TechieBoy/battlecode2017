...
//                            }
                        ...
                        else if (robotInfo.getType() == RobotType.GARDENER)
                        {
                            ...
                            out:while(rc.canSenseRobot(robotInfo.ID))
                            {
                                if (rc.canFireSingleShot() && rc.getTreeCount() > 1)
                                {
                                    rc.fireSingleShot(rc.getLocation().directionTo(robotInfo.location));
                        ...
                        {
                            if (here.isWithinDistance(robotInfo.getLocation(), GameConstants.LUMBERJACK_STRIKE_RADIUS * 4))
                            {
                        ...
                        {
                            if (here.isWithinDistance(robotInfo.getLocation(), RobotType.SOLDIER.sensorRadius))
                            {
                                tryMove(robotInfo.location.directionTo(here));
                            }
                        }
