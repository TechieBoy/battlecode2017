...
import static UnstableBot.Messaging.*;
...
//                            bugNav(visibleEnemies[0].location);
    ...
    private static void wander() throws GameActionException
    {
        if (!rc.hasMoved())
        {
            MapLocation[] enemyLocations = rc.senseBroadcastingRobotLocations();
            here = rc.getLocation();
            if(enemyLocations!=null && enemyLocations.length>0)
            {
                tryMove(here.directionTo(enemyLocations[0]));
            }
            else if (!rc.canMove(bounce))
            {
